...
isUnique x list = length (filter ((==)x) list) == 1

countUniqueDigits :: Int -> Int
countUniqueDigits n = length (filter (\x -> isUnique x y) y )
    where y = todigits n 
...
isgood :: Int -> Int -> Bool 
isgood x pos
    | even pos && even x = True
    | odd pos && isprime x = True
    | otherwise = False


isgoodlist :: [Int] -> Bool 
isgoodlist ls = and [ isgood x pos | (x, pos) <- lsWithPos]
    where lsWithPos = zip ls [0..]
...
increaseByPosition ls = [x + pos | (x, pos) <- zip ls [0..]]
...
-- rev_nums :: [Int] -> [Int]
-- rev_nums ls = map (\x -> read(reverse (show x))::Int) ls 
...
passedStudents ls x = [ name | (name, points) <- ls, sum(map (\x -> floor x) points) >= x]
...
eliminate [] = []
eliminate [x] = [x]
eliminate ls = eliminate (eliminateHelper ls)
...
-- genFib :: Int -> [Int]
-- genFib 0 = 0
-- genFib x = [fib x] ++ genFib (x-1)

-- fibList :: [Int] -> [[Int]]
-- fibList map (\x -> genFib 0 x) ls
...
carrylessDigitAddition (x:xs) (y:ys) = (x+y) `rem` 10 : carrylessDigitAddition xs ys
...
zipWithLCM ls1 ls2 = [( a, b, lcm a b) | (a, b) <- zip ls1 ls2]
...
funNum :: Int -> Int
funNum n = read( drop (length ls `div` 2) ls ++ take (length ls `div` 2) ls) ::Int
    where ls = show n 
...
foldIfTrue pred option list 
    | option == "max" = maximum filteredList
    | option == "min" = minimum filteredList
    | option == "*" = product filteredList

    | option == "+" = sum filteredList
    where filteredList = filter pred list 


-- main  = print(foldIfTrue ((>) 5) "max" [6, 1, 2, 3]) -- 6 -- check that (>)
...
averageSalary ls = fromIntegral (sum [ salo `div` length ls | (name, age, salo) <- ls]) / fromIntegral (length ls)
...
savedMoney ls = sum[ 0.15 * fromIntegral salo | (name, age, salo) <- ls, age < 35]
...
namesOlder35 ls = [ name |  (name, age, salo) <- ls, age > 15 && salo > 300 ]
