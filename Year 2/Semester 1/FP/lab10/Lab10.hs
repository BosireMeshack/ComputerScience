...
-- main :: IO ()
-- Definition of a binary tree (a for any type)
-- the pipe | operator just means that the tree has multiple constructors
...
--1. Count the number of nodes in a tree (non node)
sizeT :: Tree a -> Int
sizeT Leaf = 0
...
--2. Calculate the depth of a tree
--depthT :: Tree a -> Int
...
--3. Given a tree with key of type Int, find the sum of its nodes (leaf is 0)
sumT :: Tree Int -> Int
sumT Leaf = 0
...
inorder :: Tree a -> [a]
inorder Leaf = []
inorder (Node a le ri) = inorder le ++ [a] ++ inorder ri
...
preorder :: Tree a -> [a]
preorder Leaf = []
preorder (Node a le ri) = [a] ++ preorder le ++ preorder ri 
...
postorder :: Tree a -> [a]
postorder Leaf = []
postorder (Node a le ri) = postorder le ++ preorder ri ++ [a] 
...
task1 tree list = and (map (\x -> x `elem` (inorder tree)) list)
...
task2 Leaf num = 0
task2 (Node x le ri) num
    | x == num = 1 + task2 le num + task2 ri num
    | otherwise = task2 le num + task2 ri num

task21 :: Tree Int -> Int -> Int
task21 tree n = length (filter (==n) (inorder (tree)))
    

-- main = do
--     print $ task21 tree3fiveTimes 3 -- 5
...
task3 Leaf = []
task3 (Node x le ri) 
    | odd x =[(x, extractN le, extractN ri)] ++ task3 le ++ task3 ri
    | otherwise = task3 le ++ task3 ri
...
searchT tree num = num `elem` (filter (\x -> x == num) (inorder tree))
...
f8 Leaf _ = 0
f8 (Node x le ri) num
    | x == num = exNode le + exNode ri + f8 le num + f8 ri num
    | otherwise = f8 le num + f8 ri num
...
-- parallel combinators - 2
-- records - 3
-- trees and instances 2/3 show and eq sth like homework
-- write my own algebraic data type, show or equal for the type
-- traversing,, check deepeness, height, cut branches, basic manipulation
