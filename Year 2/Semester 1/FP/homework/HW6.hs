...
processPeople list = [(age, income* 8 `div` 10) | (age, income) <- list, age >= 25,  income >= 30000]
...
maxDiff :: (Int, Int) -> (Int, Int) -> (Int, Int)
maxDiff (maxIdx, maxDiffVal) (idx, diff)
  | diff > maxDiffVal = (idx, diff) 
  | otherwise = (maxIdx, maxDiffVal)

dayWithMaxDifference :: [(Int, Int)] -> Int
dayWithMaxDifference [] = -1 
dayWithMaxDifference temps = fst $ foldl maxDiff (-1, -1) (zip [0..] (map (\(high, low) -> high - low) temps))
...
capitalizeWords sentence = unwords [capitalize word | word <- words sentence]
...
isVowel :: Char -> Bool
isVowel a = a == 'A' || a == 'E' ||  a == 'I' || a == 'O' ||  a == 'U'

countVowelsAndConsonants :: String -> (Int, Int)
countVowelsAndConsonants word = (length [ch | ch <- word, isAlpha ch ,isVowel $ toUpper(ch)], 
     length [ch | ch <- word,isAlpha ch ,not (isVowel $ toUpper(ch))])

hw7 :: [String] -> [(Int,Int)]
hw7 wordsList = [countVowelsAndConsonants word | word <- wordsList]
