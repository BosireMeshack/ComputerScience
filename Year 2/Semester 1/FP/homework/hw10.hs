...
treeToList Leaf = []
treeToList (Node x le ri) = treeToList le ++ [x] ++ treeToList ri

ls1 = treeToList tree1

reverseList :: [Int] -> [Int]
reverseList [] = []
reverseList (x:xs) = reverseList xs ++ [x]

-- main = print $ reverseList ls1

reverseTree :: Tree a -> Tree a
reverseTree Leaf = Leaf
reverseTree (Node a le ri) = Node a (reverseTree ri) (reverseTree le)

-- main = print $ reverseTree tree1
...
getLevel n Leaf = 0
getLevel n (Node x le ri)
    | n == x = minimum(getLevel n le, getLevel n ri)
    | otherwise = 1 + getLevel n le + getLevel n ri
