...
{- 2
 ...
 If the first argument of "transformer" is unknown (which means it is not from ["add", "multiply", "power"], abort the function.
...
transformer "add" n = (\x -> x + n)
transformer "multiply" n = (\x -> x * n)
transformer "power" n = (\x -> x ^ n)
transformer _ _ = error "Unknown function"
...
-- main = print(transformer "add" 5 3)
...
-- logicChain a "add" = foldr (&&) True map ($ x) 
