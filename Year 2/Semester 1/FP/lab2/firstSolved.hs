...
-- repeat of the same for minimum
mini :: Int -> Int -> Int
mini a b
    |a < b = a 
    |otherwise = b
-- main  =  print( mini 12 13)

-- 2. Triple a number.
triple :: Int -> Int
triple x = 3 * x

-- main = print( triple 10 ) -- 30
...
-- main = print(divi 10 5)

-- main = print (divBy 0 2) -- True
...
sameRem a b c = (a `mod` c) == (mod b c)

-- main = print( sameRem 12 4 4) -- True
-- main = print( sameRem 12 4 3) -- False
-- main = print( sameRem 13 4 3) -- True

-- 9. Given two integers and a boolean value, check if the first integer is even,
-- the second divisible by 13 and the boolean value is True.
...
-- powerrec 2 4 = 2 * 2 * 2 * 2 * 1 = 16

pwr :: Int -> Int -> Int
pwr x n
  | n < 0 = error "No negative power"
  | n == 0 = 1
  | otherwise = x * pwr x(n-1)

-- main = print(pwr 2 6)


-- main = print( powerrec 2 4) --  1
...
-- Print the name of the number between 0 to 5 in characters, otherwise return "Not less or equal to 5".
...
transform days = show years ++ " years " ++ show weeks ++ " weeks " ++ show remainingDays ++ " days"
  where
    years = days `div` 365
...
digitSum n = n `mod` 10 + digitSum (n `div` 10)
    ...
    -- digitSum 1234 = 4 + 3 + 2 + 1 + 0 = 10

-- main = print(digitSum 2202176) -- 20
...
sumOdd n 
    | n <= 0 = error "has to be positive"
    | n == 1 = 1
    | (n > 0) && even n = sumOdd (n - 1)
    | (n > 0) && odd n = n + sumOdd (n - 2)
...
-- 24. Compute for a given positive n the sum of 2i*(2i+1), for i from 1 to i. E.g. for i=3 the sum is 68.
f :: Int -> Int
f i
  | i == 0 = 0
  | i == 1 = 6
  | otherwise = 2*i*(2*i+1) + f(i-1)
