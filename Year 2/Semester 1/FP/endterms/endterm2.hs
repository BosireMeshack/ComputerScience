...
-- helper:: Circle -> Point -> Bool
-- helper crc pt = sqrt(((x (center crc))- (x pt))^2 + )


-- check :: Circle -> [Point] -> Bool
-- check circ ptArr = map
...
data Library = Library { libName :: String, books :: [Book] } 
...
instance Eq Book where
    book1 == book2 = (title book1) == (title book2) && (author book1) == (author book2) && (pyear book1) == (pyear book2) && (numOfPages book1) == (numOfPages book2)
...
instance Ord Book where
    book1 < book2 = ((pyear book1) < (pyear book2))
-- main = print $ b1 < b2 -- False
...
instance Show Library where 
    show (Library n1 l1) = "Name:" ++ n1  

-- main = print(show lib1)
...
-}
helperLib :: Library -> Library -> Bool
helperLib l1 l2 = (and [elem li (books l2) | li <- (books l1)]) && (and [elem li (books l1) | li <- (books l2)])

instance Eq Library where
    libb1 == libb2 = helperLib libb1 libb2
...
getDepth Leaf = 0
getDepth (Node x le ri) = 1 + (getDepth le) + (getDepth ri)

-- main = print $ getDepth tree1
avl_prop_check :: Tree Int -> Bool
avl_prop_check Leaf = True
avl_prop_check (Node x le ri) = (abs ((getDepth le) - (getDepth ri)) <=1)
...
toTupler:: [Int] -> [(Int, Int)]
toTupler ls = [(x, x+ sum (filter (\y -> x < y) ls)) | x <-ls]
...
filter123 :: [(Int, Int, Int)] -> [(Int, Int, Int)]
filter123 tp = filter (\(x,y,z) -> x /= y && y /= z && z /=x) tp

getSum :: [(Int, Int, Int)] -> Int
getSum ls = minimum (map(\(x,y,z) -> abs(x+y+z)) ls)



alterArray :: [(Int, Int, Int)] -> [(Int, Int, Int)]
alterArray ls = (filter123 ls)
