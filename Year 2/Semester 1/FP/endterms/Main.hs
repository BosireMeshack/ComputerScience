...
bestBscStudent :: [Student] -> String
bestBscStudent [] = ""
bestBscStudent ls = head [idS s | s <- ls, (sum (grades s) `div` (length (grades s))) == bestgrade && (level s) == Bsc]
    where bestgrade = maximum [sum (grades x) `div` length (grades x) | x <- ls]
          

-- main = print $ bestBscStudent [st1]
-- main = print $ bestBscStudent [st1, st4, st5]
-- Eq(==,/=)
instance Eq StudyLevel where
    Bsc == Bsc = True
    Msc == Msc = True
    PhD == PhD = True
    _ == _ = False
 ...
 -- average   
avgg :: [Int] -> Float
avgg l = (fromIntegral (sum l)) / (fromIntegral (length l) )


instance Eq Student where
    (Student i1 l1 g1) == (Student i2 l2 g2) = ((avgg g1) == (avgg g2)) && (i1 == i2) && (l1==l2)
    

-- Ord(<,>,<=,>=)
instance Ord Student where
    (Student i1 l1 g1) < (Student i2 l2 g2) = (avgg g1) < (avgg g2)
    (Student i1 l1 g1) <= (Student i2 l2 g2) = (avgg g1) < (avgg g2) || (avgg g1) == (avgg g2)
    
--
st1 = Student {idS="st-1", level=Bsc, grades=[3,4,3]}
st2 = Student {idS="st-2", level=Msc, grades=[3,1,3]}
st3 = Student {idS="st-3", level=PhD, grades=[5]}
st4 = Student {idS="st-4", level=Bsc, grades=[5,5,4]}
st5 = Student {idS="st-5", level=Bsc, grades=[5,5,5,2,4]}

getBestBScStudent :: [Student] -> String
getBestBScStudent list =  idS (last (sort [x | x<-list , ((level x)==Bsc)]))

-- main = print $ getBestBScStudent [st1] -- "st-1"
--main = getBestBScStudent [st1, st4, st5] -- "st-4"
--main = getBestBScStudent [st2, st3, st5] -- "st-5"
-- main = print (getBestBScStudent [st1, st2, st3, st4, st5]) -- "st-4"
...
countOccChar :: String -> Char -> Int 
countOccChar s c = length [a | a<-s, a==c]

-- "abcaa" 'a' ['a','a','a']

count :: String -> [(Char, Int)]
count s = nub [(c, (countOccChar s c)) | c<-s, (countOccChar s c)>1]

-- main = print (count "thequickbrownfoxjumpsoverthelazydog") -- [('t',2),('h',2),('e',3),('u',2),('r',2),('o',4)]
-- main = print (count "Helloworld") -- [('l',3),('o',2)]
...
-- main = print (getBTHeight bt3) -- 5
-- main = print (getBTHeight bt4) -- 7
...
isNil :: LinkedList String -> Bool
isNil Nil = True
isNil (Pointer x next) = False


insertL :: (LinkedList String) -> String -> (LinkedList String)
insertL Nil s = (Pointer s Nil)
insertL (Pointer x next) s 
  | isNil next = (Pointer x (Pointer s Nil))
  | otherwise = (Pointer x (insertL next s))
  


-- main = print (insertL linkedlist1 "World") -- (Pointer "o" (Pointer "l" (Pointer "l" (Pointer "e" (Pointer "H" (Pointer "World" Nil))))))
...
concatt ::(LinkedList String) -> (LinkedList String) -> (LinkedList String)
concatt Nil (Pointer x2 next2) = (Pointer x2 next2) 
concatt (Pointer x1 next1) Nil = (Pointer x1 next1)
concatt (Pointer x1 next1) (Pointer x2 next2)
  |isNil next1=(Pointer x1 (Pointer x2 next2))
  |otherwise = (Pointer x1(concatt next1(Pointer x2 next2)))

-- main = print (concatt linkedlist1 linkedlist2 )-- (Pointer "o" (Pointer "l" (Pointer "l" (Pointer "e" (Pointer "H" (Pointer "y" (Pointer "e" (Pointer "H" Nil))))))))
-- main = print (concatt linkedlist2 linkedlist2) -- (Pointer "y" (Pointer "e" (Pointer "H" (Pointer "y" (Pointer "e" (Pointer "H" Nil))))))
...
instance Eq BasicPersonAccount where
  (BasicPersonAccount n1 _ _ ip1) == (BasicPersonAccount n2 _ _ ip2) = (n1==n2)  && (ip1 == ip2) 

instance Show BasicPersonAccount where
  show (BasicPersonAccount n1 a1 i1 ip1) = n1 ++ " " ++ show a1 ++ " " ++ show i1 ++ " " ++ show ip1 

findFakes :: [BasicPersonAccount] -> [BasicPersonAccount]
findFakes [] = []
findFakes (x:y:xs)
    | x == y = x {inQueue = False} : findFakes xs
    | otherwise = x : y  : findFakes xs


processAccount :: BasicPersonAccount -> [BasicPersonAccount] -> Bool
processAccount acc [] = False
processAccount acc (y:ys)
  | acc == y = True
  | otherwise = processAccount acc ys



updateAccount :: BasicPersonAccount -> BasicPersonAccount
updateAccount acc  = acc {inQueue = False}
...
isA :: OneOf String Char -> Bool
isA (A _ ) = True
isA (B _) = False

calcAB :: OneOf String Char -> Int
calcAB (A x) = length x
calcAB (B y) = 1
  
findWhich :: [(OneOf String Char)] -> Int
findWhich list = sum[calcAB x | x<-list]
