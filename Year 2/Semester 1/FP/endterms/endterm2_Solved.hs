...
shiftCipher word shift = [shif c | c <- word]
    where
        cipher =  "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
        shif c = (dropWhile (/= c) cipher ++ takeWhile (/= c) cipher) !! shift
...
check c l = and [canReach loc | loc <- l]
    where
        canReach loc = (x loc -  x (center c))^2 + (y loc -  y (center c))^2 <= 25

-- check1 :: Circle -> [Point] -> Bool
-- check1 c l [distance loc | loc <- l]
--     where distance = (x loc - x (center c))^2 + (y loc - y (center c))^2 



-- elem c (filter (\x -> (distance < 25) x) l)
...
instance Eq Book where
        (==) :: Book -> Book -> Bool
        (==) b1 b2 = title b1 == title b2 && author b1 == author b2 && pyear b1 == pyear b2 && numOfPages b1 == numOfPages b2

-- instance Eq Library where
--         (==) :: Library -> Library -> Bool
--         (==) l1b1 lib2  = libName lib1 == libName lib2 
...
instance Eq Library where
    (==) :: Library -> Library -> Bool
    (==) lib1 lib2 = books lib1 == books lib2
...
getHeight :: Tree Int -> Int
getHeight Leaf = 0
getHeight (Node _ l r) = 1 + max (getHeight l) (getHeight r)

avl_prop_check :: Tree Int -> Bool
avl_prop_check Leaf = True
avl_prop_check (Node _ l r) = abs (getHeight l - getHeight r) <= 1 && avl_prop_check l && avl_prop_check r
...
aux :: Tree Int -> [Int] -> Tree Int
aux Leaf _ = Leaf
aux (Node val l r) tlist = Node (val + sum (filter (> val) tlist)) (aux l tlist) (aux r tlist)

transform :: Tree Int -> Tree Int
transform x = aux x tlist
    where
        tlist = treeToList x
...
-- main = print $ transform bsTree2 -- (Node 20 (Node 27 (Node 27 (Node 32 (Node 33 Leaf Leaf) Leaf) Leaf) (Node 20 Leaf Leaf)) (Node 11 (Node 11 Leaf Leaf) (Node 6 Leaf Leaf)))
...
-}
getSmallestUqinue :: [(Int, Int, Int)] -> (Int, Int, Int)
getSmallestUqinue aot = foldr (\t1 nt -> if isUnique t1 && (isSmaller t1 nt || not (isUnique nt)) then t1 else nt) (head aot) (tail aot)
    where
        isUnique (x,y,z) = x /= y && y /= z && x /= z
        isSmaller (x,y,z) (a,b,c) = abs (x+y+z) < abs (a+b+c)

alterArray :: [(Int, Int, Int)] -> [(Int, Int, Int)]
alterArray aot = [addd t uniqt | t <- aot]
    where
        uniqt = getSmallestUqinue aot
        addd (x,y,z) (a,b,c) = (x*a, y*b, z*c)
...
instance Ord EngagedPerson where
    (<=):: EngagedPerson -> EngagedPerson -> Bool
    (<=) e1 e2 = (yearOfMarriage e1) <= (yearOfMarriage e2)

instance Eq EngagedPerson where
    (==):: EngagedPerson -> EngagedPerson -> Bool
    (==) e1 e2 = (yearOfMarriage e1) == (yearOfMarriage e2)


getOldestMarriage :: [(EngagedPerson, EngagedPerson)] -> String
getOldestMarriage lc = tail $ dropWhile (/= ' ') $ fullName $ fst (foldr (\(p1, p2) (p3, p4) -> if yearOfMarriage p1 < yearOfMarriage p3 then (p1, p2) else (p3, p4)) (head lc) (tail lc))
...
neighbours word = [shif c | c <- word]
    where
        cipher =  [Indigo, Orange, Yellow, Green, Blue, Indigo, Violet]
        shif c = (dropWhile (/= c) cipher ++ takeWhile (/= c) cipher) !! 2
...
getDepth :: BinaryTree2 -> (Int, Int)
getDepth BTLeaf2 = (1, 0)
getDepth (BTNode2 left right) =
    (max (fst lDepDia) (fst rDepDia) + 1,
    max (snd lDepDia) (max (snd rDepDia) (fst lDepDia + fst rDepDia)))
    where
    lDepDia = getDepth left
    rDepDia = getDepth right

getBTDiameter :: BinaryTree2 -> Int
getBTDiameter root = snd (getDepth root) + 1
