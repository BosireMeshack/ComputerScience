...
isTriangle origo point1 point2 
    | (base + height == hypotenuse) || hypotenuse+base == height || height + hypotenuse == base && v = True
    | otherwise = False
    where base = (x origo - x point1) * (x origo - x point1) + (y origo - y point1) * (y origo - y point1)
          height = (x point2 - x origo) * (x point2 - x origo) + (y point2 - y origo) * (y point2 - y origo)
          hypotenuse = ((x point2 - x point1)*(x point2 - x point1)) + ((y point2 - y point1)*(y point2 - y point1))
          v = visible origo && visible point1 && visible point2
...
changeHeight mtu 
    | tall mtu > 1.70 = mtu {tall = (tall mtu - 0.01* tall mtu)} 
    | otherwise = mtu
...
getAvgGrade s 
    | null (grades s) = 0
    |otherwise = fromIntegral (sum (grades s)) / fromIntegral (length (grades s))

-- main = print $ getAvgGrade (students!!1)

getBestStudents :: [Student] -> [Student]
getBestStudents ls = [x | x <- ls, getAvgGrade x == highestgrade]
    where highestgrade = maximum [ getAvgGrade x | x <- ls]
...
shouldWePlay ls gp 
    | length goalscored >= 3 = True 
    | otherwise = False
    where goalscored = [x | x<- ls, skillLevel x >= skillLevel gp]
...
checkI :: Country -> Bool
checkI ctry = isPrime (length [i | i <- capital ctry, i=='i'])
 
checkCount :: Continent -> Bool
checkCount cont = length (filter (==True) (map (\x -> checkI x) (countries cont))) > 0

 
-- Main function
continentsPrimeI :: [Continent] -> [String]
continentsPrimeI ls = [ contName x | x <- ls, checkCount x]
