...
-- f1 [] = []
-- f1 (x:xs)
--     | x `mod` 3 == 0 && x `mod` 5 == 0 = x : f1 xs
--     | otherwise = f1 xs 

ls = [2,15, 45, 50]
f3f5 :: Int -> Bool
f3f5 x = x `mod` 3 == 0 && x `mod` 5 == 0

-- f5 :: Int -> Bool
-- f5 x = x `mod` 5 == 0

r = foldl (+) 0 (filter (f3f5) ls)


hof1 :: [Int] -> Int
hof1 xs = foldl (+) 0 $ filter (f3f5) xs
...
-- why we have a space at the end 
...
l7 :: [(Int, Int, Int)]
l7 = take 100 [(a,b,c) | c <-[1..], b <- [1..c], a <- [1..b]]

-- just c is restricted, not the number of pairs
l71 = take 100[ (a,b,c) | c <- [1..], b<-[1..c], a<-[1..b] , a*a + b*b == c*c ]
...
l15 :: Bool
l15 = length [x | x <- [1..123457], 123457 `mod` 2 ==0] == 2
--main = print(l15)
...
sumtup tup = (sum (fst (unzip tup)), sum (snd (unzip tup)))
...
l30 :: [(Int, Int)]
l30 = zip [1..12] [31,28,31,30,31,30,31,31,30,31,30,31]
