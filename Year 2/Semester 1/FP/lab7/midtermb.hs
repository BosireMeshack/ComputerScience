...
countStepsToOne x 
    | x ==  1 = 0
    | x `mod` 3 == 0 = 1 + countStepsToOne (x `div` 3)
    | x `mod` 2 == 0 = 1 + countStepsToOne (x `div` 2)
    | otherwise = 1 + countStepsToOne (x - 1)
...
helperindex [] num ind = -1
helperindex (x:xs) num ind
    | x == num = ind
    | otherwise = helperindex xs num (ind + 1)
...
todigit x 
    | x< 10 = [x]
    | otherwise = [x `mod` 10] ++ todigit (x `div` 10)
...
splitsum :: Int -> [Int]
splitsum n = helpersum (todigit n) (length (todigit n) - 1) 1

helpersum :: [Int] -> Int -> Int -> [Int] 
helpersum [] len ind = []
helpersum ls len ind
    | ind <= len = [(toInt (take ind ls))+ (toInt (drop ind ls))]
    | otherwise = []
...
apply :: (a -> a -> a) -> [a] -> [a] -> [a]
-- apply fx x y = map (\tup -> fx (fst tup)  (snd tup)) (zip x y)
apply fx x y = zipWith fx x y
...
findPairSums ls1 ls2 n = [(x,y) | x <- ls1 , y <- ls2 , x + y == n ]
...
update key val (x:xs)
    | key == fst x = (key, val) : update key val xs
    | otherwise update key val xs
