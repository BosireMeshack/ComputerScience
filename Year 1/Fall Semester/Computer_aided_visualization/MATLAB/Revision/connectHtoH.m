...
t1 = 1; t2 = 2;
e = [x(t1) == px(1), x(t2) == px(2), xd(t1) == vx(1) , xd(t2) == vx(2)];
...
t1 = 1; t2 = 2;
e = [y(t1) == py(1), y(t2) == py(2), yd(t1) == vy(1) , yd(t2) == vy(2)];
m = solve(e, [b3 b2 b1 b0]);
cy(t) = subs(y, [b3 b2 b1 b0], [m.b3 m.b2 m.b1 m.b0]);
fplot(cx, cy, [t1 t2]);
%%%%%%%%%CONNCECTING HERMITE 2P-2V CURVES C1 Continuity tangents are the same%%%%%%%%%%%%%
rx = [px(2) 10];
ry = [py(2) -1];
plot(rx, ry, "*m");
ux = [vx(2) 0];
...
t3 = -1;
t4 = 1;
e = [x(t3) == rx(1), x(t4) == rx(2), xd(t3) == ux(1), xd(t4) == ux(2)];
m = solve(e, [a3 a2 a1 a0]);
dx(t) = subs(x, [a3 a2 a1 a0], [m.a3 m.a2 m.a1 m.a0]);
e = [y(t3) == ry(1), y(t4) == ry(2), yd(t3) == uy(1), yd(t4) == uy(2)];
m = solve(e, [b3 b2 b1 b0]);
